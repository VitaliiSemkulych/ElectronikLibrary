...
import com.example.demo.extractor.MarckExtractor;
...
public class BookDAO {
    ...
    @Autowired
    private MarckExtractor marckExtractor;
    ...
    public List<Book> selectByFrace(SearchByPhraseModel searchFrace) {
        ...
        if(searchFrace.getSearchType()==SearchType.AUTHOR){
            selectByLetter+="Where A.fio Like '%" + searchFrace.getSearchPhrace().toLowerCase() + "%' ORDER BY B.name";
        }else{
            selectByLetter+="Where B.name Like '%" +searchFrace.getSearchPhrace().toLowerCase() + "%' ORDER BY B.name";
        }
        return this.getBookList(selectByLetter);
    }
    ...
    public List<Marck> getMarckList(long bookId){
        String SQL="select mark from evaluation where book_id=?";
        return jdbcTemplate.query(SQL,new Object[]{bookId},marckExtractor);
    }
