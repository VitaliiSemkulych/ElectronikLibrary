...
public class SingleBookController {
    ...
    public String insertBookInfo(Model model, @RequestParam("id") long bookId,HttpSession session){
        ...
        if(bookDAO.getReadingModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
        model.addAttribute("inReadingMode",true);
        }else{
            model.addAttribute("inReadingMode",false);
        }
        if(bookDAO.getInterestingModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
            model.addAttribute("inInterestingMode",true);
        }else{
            model.addAttribute("inInterestingMode",false);
        }
        if(bookDAO.getReadModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
            model.addAttribute("inReadMode",true);
        }else{
            model.addAttribute("inReadMode",false);
        }
        if(bookDAO.getMarkNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
        ...
        List<Marck> marckList=bookDAO.getMarckList(bookId);
        if (marckList.isEmpty()){
            model.addAttribute("bookMarck","Not evaluated yet");
        }else {
            int summ=marckList.stream().mapToInt(o->o.getMarck()).sum();
            model.addAttribute("bookMarck",String.valueOf(new DecimalFormat("#.##")
                    .format(summ/marckList.size())));
    ...
    public String changeInterestingMode(RedirectAttributes redirectAttributes, @RequestParam("id") long bookId, HttpSession session){
        ...
        if(bookDAO.getInterestingModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
            bookDAO.delateBookInInterestingMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }else{
            bookDAO.addBookInInterestingMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }
        redirectAttributes.addAttribute("id",bookId);
        return "redirect:/book";
    }
    ...
    public String changeReadingMode(RedirectAttributes redirectAttributes, @RequestParam("id") long bookId, HttpSession session){
        ...
        if(bookDAO.getReadingModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
            bookDAO.delateBookInReadingMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }else{
            bookDAO.addBookInReadingMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }
        redirectAttributes.addAttribute("id",bookId);
        return "redirect:/book";
    }
    ...
    public String changeReadMode(RedirectAttributes redirectAttributes, @RequestParam("id") long bookId, HttpSession session){
        ...
        if(bookDAO.getReadModeNumber(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail())!=0){
            bookDAO.delateBookInReadMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }else{
            bookDAO.addBookInReadMode(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        }
        redirectAttributes.addAttribute("id",bookId);
        return "redirect:/book";
    }
    ...
    public String evaluate(RedirectAttributes redirectAttributes, @RequestParam("id") long bookId,@RequestParam("marckValue") int marck, HttpSession session){
            bookDAO.updateMark(bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail(),marck);
        redirectAttributes.addAttribute("id",bookId);
        return "redirect:/book";
    }
    ...
    public String sendRecension(HttpSession session,RedirectAttributes redirectAttributes,
        ...
        recencionDAO.sendRecension(recensionText,bookId,((UserRegistrForm)session.getAttribute("lodinUser")).getUserEmail());
        redirectAttributes.addAttribute("id",bookId);
        return "redirect:/book";
    }
